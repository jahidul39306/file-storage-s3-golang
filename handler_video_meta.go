...
func (cfg *apiConfig) handlerVideoGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Unable to generate presignedURL for video for fetching the video", err)
		return
	}
...
func (cfg *apiConfig) handlerVideosRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	for i, v := range videos {
		...
		if err != nil {
			respondWithError(w, http.StatusInternalServerError, "Unable to generate presignedURL for fetching many videos", err)
			return
		}
		videos[i] = video
	}
